...
class MemoryCostModel:
    ...
    def __post_init__(self, stategy, global_batch_size: int = 8, mbsz: int = -1, min_tp: int = -1, max_tp: int = -1, stage_idx: int = 0, vsp: int = 0, embed_sdp: bool = False,
        ...
        print(f"args.vsp set {self.args.vsp}")
    ...
    def estimate_other_memory_cost(self):
        ...
        for tp in total_min_tp:
            ...
            if self.pp_size == 1:
                ssss = args.other_memory_pp_off['model_states'][model_tp]
                print(f'model_tp is {ssss}')
                res = args.other_memory_pp_off['model_states'][model_tp] * other_ms_zero2_ratio
                print(f'res is {res}')
            ...
            for i in range(len(tp_other_memory_cost)):
                tp_other_memory_cost[i] += args.pytorch_context_mem  # 此处添加了超参数
