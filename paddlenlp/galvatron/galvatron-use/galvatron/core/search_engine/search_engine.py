...
class GalvatronSearchEngine():
    ...
    def parallelism_optimization(self):
        ...
        def search_for_chunk(bsz, chunk, min_tp, max_tp, vsp, embed_sdp):
            ...
            for sp_search in sp_search_speace:
                ...
                strategies = [s for s in temp_strategies if min_tp <= s[1] and max_tp >= s[1]]  # 将tp在[min_tp, max_tp]范围内的策略进行保留
                strategies = [s for s in strategies if chunk <= bsz // (self.args.gpu_num // s[0] // min_tp) ]  # 将满足不等式的策略进行保留
                if sp_search == 1:  # 将有sp标志,但是使用了sp(即ulysses)的策略排除
                    strategies = [s for s in strategies if 'sp' not in s[-1] or ('sp' in s[-1] and s[-1]['sp'] == 0)]
                if sp_search == 2: # 将有sp标志,但是没使用sp(即ulysses)的策略排除
                    strategies = [s for s in strategies if 'sp' not in s[-1] or ('sp' in s[-1] and s[-1]['sp'] == 1)]
                if len(strategies) == 0:
                    continue
                
                pp_deg_list = sorted(list(set([s[0] for s in strategies])))  # 根据pp_degree进行排序
                
                pp_deg_list = [pp for pp in pp_deg_list if pp * min_tp <= self.args.gpu_num and bsz % (self.args.gpu_num // pp // min_tp) == 0]  # 计算出最大的dp_degree,若bsz能够除尽最大的dp_degree,则可以除尽所有的dp_degree,若最大的dp_degree无法除尽,则会影响能考虑的情况,故而排除掉
                
                if len(pp_deg_list) == 0:
                    continue
                
                strategies = [s for s in strategies if s[0] in pp_deg_list]  # 将满足pp要求的策略进行保留
                
                mbsz_dict = dict() # calc micro batch size in different pp size when tp = min_tp
                for pp in pp_deg_list:
                    mbsz_dict[pp] = (bsz // (self.args.gpu_num // pp // min_tp) + chunk - 1) // chunk  # 在指定chunk和pp下,考虑最大的dp,计算出mbsz
                
                # strict mode: search chunk must be equal to real chunk # [note] 真的存在这种情况吗?
        ...
        if self.args.parallel_search:
            ...
            for bsz in self.BSZs:
                results[bsz] = dict()
                chunk_list = range(1,bsz+1)  # 累积步数首先是可以全部的,注意，此处的chunk只是说允许的chunk上限,而不是实际的真正的chunk
                # assert(bsz % self.args.gpu_num == 0), "bdz should be divisible by world size"
                if self.args.settle_chunk != -1:
                    chunk_list = [self.args.settle_chunk]
                for chunk in chunk_list:
                    results[bsz][chunk] = dict()
                    if bsz % chunk != 0:  # 只有 bsz % chunk == 0 的时候才会进行搜索
                        continue
                    for min_tp in total_min_tp:
                        results[bsz][chunk][min_tp] = dict()
                        for max_tp in total_max_tp:
    ...
    def check_cost_model(self, bsz, chunk, min_tp):
        ...
        strategies = [s for s in strategies if s[1] >= min_tp]  # 将tp >= min_tp的策略进行保留
        print(strategies)
        pp_deg_list = sorted(list(set([s[0] for s in strategies])))
        
        pp_deg_list = [pp for pp in pp_deg_list if pp * min_tp <= self.args.gpu_num and bsz % (self.args.gpu_num // pp // min_tp) == 0]  # 将满足要求的pp进行保留
        ...
        for i in range(self.num_layertype):
            ...
            for strategy in strategies:  # [note] 此处需要进行更改
                re = MemoryCostModel(strategy, global_batch_size=bsz, mbsz = mbsz_dict[strategy[0]], min_tp = min_tp, max_tp=min_tp,  # [note] 更改
    ...
    # 用于毕业
    def check_cost_model(self, bsz, chunk, min_tp, strategy):
        memory = [[] for _ in range(self.num_layertype)]
        memory_total = [[] for _ in range(self.num_layertype)]
        # strategies = copy.deepcopy(self.strategies)
        ...
        # 2-2-2-zero2
        bsz, chunk, min_tp = 64, 8, 2
        strategies = [[2, 2, 2, {'tp': 1, 'fsdp': 0, 'sp': 0}]]  # [note]先手动设定
        
        # 8-1-1
        # bsz, chunk, min_tp = 64, 16, 1
        # strategies = [[8, 1, 1, {'sp': 0}]]  # [note]先手动设定
        
        # 8-1-1
        # bsz, chunk, min_tp = 64, 32, 1
        # strategies = [[8, 1, 1, {'sp': 0}]]  # [note]先手动设定
        
        # 1-8-1
        # bsz, chunk, min_tp = 16, 4, 8
        # strategies = [[1, 8, 1, {'tp':1, 'sp': 0]]  # [note]先手动设定
        
        
        # 1-1-8-zero2
        # bsz, chunk, min_tp = 64, 8, 1
        # strategies = [[1, 1, 8, {'tp':1, 'sp': 0, 'fsdp':0}]]  # [note]先手动设定
        
        # 1-1-8-zero3
        # bsz, chunk, min_tp = 32, 4, 1
        # strategies = [[1, 1, 8, {'tp':1, 'sp': 0, 'fsdp':1}]]  # [note]先手动设定
        
        # 1-1-8-ddp
        bsz, chunk, min_tp = 64, 8, 1
        strategies = [[1, 1, 8, {'tp':1, 'sp': 0}]]  # [note]先手动设定
        ...
        for i in range(self.num_layertype):
            model_args, train_args, parallel_args, profile_model_args, profile_hardware_args, layer_num = self.model_args_list[i], self.train_args_list[i], self.parallel_args_list[i], self.profile_model_args_list[i], self.profile_hardware_args_list[i], self.layernum_list[i]
            for strategy in strategies:  # [note] 此处需要进行更改
                re = MemoryCostModel(strategy, global_batch_size=bsz, mbsz = mbsz_dict[strategy[0]], min_tp = min_tp, max_tp=min_tp, vsp=0, embed_sdp=False, # [note] 更改
                                     model_args=model_args, train_args=train_args, parallel_args=parallel_args, profile_model_args=profile_model_args).get_memory_cost()
                re_total = re['enc_total']*layer_num/strategy[0]
                print(form_strategy(strategy), re['enc_total'], re['other'], [re_total + re_other for re_other in re['other'][min_tp]])
                memory[i].append(re['enc_total'])
