...
class allreduce_block(nn.Layer):
    def __init__(self, mp_group):
        ...
        # self.linear = nn.Linear(1024, 1024)  # 这个貌似没有使用到
...
def fake_loss_func(labels, outputs):  # 该函数注意labels和outputs的位置,以及是否需要返回两个loss
    loss = outputs.sum()
...
def train(args):
    ...
    # tp_groups = []  # TODO 此处需要设置一下tp_groups 分别设置每一层都具有一个通信组和每一层共用一个通信组
    tp_group = paddle.distributed.new_group(ranks=[i for i in range(world_size)])  # 先进行这样的一个测试 也就是tp_size = 8
    print(f"tp_group.nranks: {tp_group.nranks}")
    
    # tp_groups = []
    # for i in range(args.num_layers):
    #     tp_groups.append(paddle.distributed.new_group(ranks=[i for i in range(world_size)]))  # 先进行这样的一个测试 也就是tp_size = 8
    
    model = nn.Sequential()
    model.add_sublayer('pre_sync_module', pre_sync_module())
    model.add_sublayer('pre_mlp', pre_mlp())
    for i in range(args.num_layers):
        # module = allreduce_block(tp_groups[i]) # 之后查看以下是否需要每一层分别设置一个通信组
    ...
    with paddle.profiler.Profiler(
        ...
        for i, input in enumerate(tqdm(dataloader)):
            ...
            p.step()   
    p.export(f"./raw_log/rank{rank}.json")
    ...
    comm_time = 0
    comm_num = 0
    with open(f"./raw_log/rank{rank}.json", "r") as f:
        data = json.load(f)
    ...
    allreduce_time_24_layer = comm_time / 10 # [note] 为什么是除以10 (因为10次迭代)
