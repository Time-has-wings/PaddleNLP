...
class GalvatronSearchEngine():
    ...
    def set_searching_bsz(self):
        ...
        if args.settle_bsz is not None and args.settle_bsz > 0:
            ...
            return
        self.bsz_scale = args.bsz_scale # 这个值就是从参数进行获取的，而不是也和min_bsz一样可以通过计算获得

        if args.recommend_min_bsz:
            recommend_bsz = self.recommend_min_bsz(self.bsz_scale) # 推荐一个最合适的min_bsz
    ...
    def recommend_min_bsz(self, scale):
        ...
        if not args.disable_dp:
            baselines.append([1,1,gpu_num,{'fsdp':0}]) # dp=wprld_size and zero2
        if not args.disable_sdp:
            baselines.append([1,1,gpu_num,{'fsdp':1}]) # dp=world_size and zero3
        if not args.disable_tp:
            baselines.append([1,gpu_num,1,{'fsdp':0}]) # tp=world_size and zero2
